...
public:
    string longestPalindrome(string _s) {
        ...
        while (center < s.size()) {
            ...
            assert(center - r <= center + r); // DEBUG
            assert(0 <= center - r && center + r < s.size()); // DEBUG
            assert(s[center - r] == s[center - r]); // DEBUG
            ...
            if (r > 0) {
                ...
                for (int r = 1, p = center + 1, m = center - 1; m >= center - R && p <= center + R; r ++, m --, p ++) {
                    // We assume that all mirrors are already solved (by DP hypothesis)
                    // If we have solved a previous palindrome we have one of there cases
                    //   1. It is in this palindrome completely and we can skip till after it
                    //      (after copying ALL the DP's inside... I think)
                    //   2. It is touching the edge of this palindrome
                    //   3. It is surpassing the edge of this palindrome
                    // Cases 2 and 3 are non-obvious:
                    //   The palindrome may or may not expand past the radial limits...
                    if (dp[m] > 0) {
        ...
        assert(0 <= max_center - max_r && max_center + max_r < s.size()); // DEBUG
        vector<char> ans;
